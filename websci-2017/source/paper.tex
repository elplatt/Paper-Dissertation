...
while more realistic than the assumption used by webs of trust,
...
When network structure can be dictated
...
This assumption arises because scalable network architectures can be made
...
those servers.
However, there may be ways to achieve improved attack-tolerance from architectures
...
with different properties used to prove independence in different stages
(see Figure \ref{fig:route-overview}).
In the first cycle (stages 2--4), path independence is guaranteed by ensuring that
all nodes match the path parameter $s$ in the first $h$ bits of the place-within-level.
Similarly, in the second cycle (stages 5--6),
...
The {\em directory authorities} used by the Tor
\cite{dingledine_tor:_2004} network are one example.
However, the size of such authority networks is often limited by the number of
trust relationships (degree) each node can maintain, and the inherent insecurity of
...
it may at times be appropriate to assume mutual trust and revoke that relationship
when it is violated.

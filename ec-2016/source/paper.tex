...
More specifically, the algorithm can be understood by dividing the bits of
$z$ into four segments: $A, B, C, D$ (Figure \ref{fig:route-overview}).
$A$ is the first $h$ bits, followed by $B$ until reaching $C$: the last $h$ bits before
the destination level $l_w$, and then finally $D$.
As the routing algorithm progresses through the levels $l$, the segments of $z$
get updated in order.
Because of vertex transitivity, we can make the simplifying assumption
that $v = (0,0)$ without loss of generality.
Initially (1) while still in the source trusted neighborhood,
$A$ is set to match $s$, which ensures that no two
paths will share nodes outside the trusted neighborhood.
Then (2) the bits of $B$ are inverted and (3) a cyclic permutation of $s$ is
placed into $C$.
After this step (4--6), $D$, $A$, and $B$ are set to their destination values.
After $A$ is set, the path remains distinct from other paths because other nodes
either have uninverted values in $B$ or a different value of $\tilde{s}$ in $C$.
While (7) $C$ is being set to the destination value, overlaps may occur, but
nodes are now within the trusted neighborhood of the destination.
...
\foreach \i in {0\ldots,{\ldots}0\ldots,{\ldots}0\ldots,{\ldots}0}% <-- content of nodes
    \node[X] {\i};
\draw[B] ( A-1.north west) -- node[above=1.5mm] {$h$} ( A-1.north east);
\draw[B] ( A-2.north west) -- node[above=1.5mm] {$l_w - 2h$} ( A-2.north east);
\draw[B] ( A-3.north west) -- node[above=1.5mm] {$h$} ( A-3.north east);
\draw[B] ( A-4.north west) -- node[above=1.5mm] {$m - l_w$} ( A-4.north east);
...
\foreach \i in {$s$,{\ldots}0\ldots,{\ldots}0\ldots,{\ldots}0}% <-- content of nodes
    \node[X] {\i};
...
\foreach \i in {$s$,{\ldots}1\ldots,{\ldots}0\ldots,{\ldots}0}% <-- content of nodes
    \node[X] {\i};
...
\foreach \i in {$s$,{\ldots}1\ldots,{\ldots}$\tilde{s}$\ldots,{\ldots}0}% <-- content of nodes
    \node[X] {\i};
...
\foreach \i in {$s$,{\ldots}1\ldots,{\ldots}$\tilde{s}$\ldots,$z_{w,D}$}% <-- content of nodes
    \node[X] {\i};
...
\foreach \i in {$z_{w,A}$,{\ldots}1\ldots,{\ldots}$\tilde{s}$\ldots,$z_{w,D}$}% <-- content of nodes
    \node[X] {\i};
...
\foreach \i in {$z_{w,A}$,$z_{w,B}$,{\ldots}$\tilde{s}$\ldots,$z_{w,D}$}% <-- content of nodes
    \node[X] {\i};
...
\foreach \i in {$z_{w,A}$,$z_{w,B}$,$z_{w,C}$,$z_{w,D}$}% <-- content of nodes
    \node[X] {\i};

...
%DIF DEL r1/NetFT-plosone-r1.tex   Sun Jan  6 10:58:37 2019
%DIF ADD r2/Manuscript.tex         Thu Jan 10 11:24:49 2019
...
redundancy-based approaches to \DIFdelbegin \DIFdel{attack tolerance.
}\DIFdelend \DIFaddbegin \DIFadd{attack-tolerance.
For the purpose of this paper, }{\em \DIFadd{attack-tolerance}} \DIFadd{refers to the ability of a pair of
communicating nodes to detect,
with high probability,
when a message has been blocked or altered
in the presence of adversarial faults.
}\DIFaddend Our primary result is theoretical: an algorithm for constructing highly redundant
...
in infrastructure, topology can be engineered and maintained as infrastructure\DIFdelbegin %DIFDELCMD < }%%%
\DIFdelend .
...
\DIFdelbegin %DIFDELCMD < \item{We prove that the number of $h$-internally vertex disjoint
%DIFDELCMD < paths between two nodes in a wrap-around butterfly network
%DIFDELCMD < is at least $2^h$,
%DIFDELCMD < and present a scalable and efficient concurrent multipath routing (CMR) algorithm
%DIFDELCMD < to find these paths,
%DIFDELCMD < which can be combined with SMFT to achieve a high level of attack-tolerance.
%DIFDELCMD < }
%DIFDELCMD < %%%
...
\DIFaddendFL %\centerline{\includegraphics{fig-perror}}
\caption{
...
We define the {\em bounded trust model} \DIFaddbegin \DIFadd{(Fig. \ref{fig:trust-source}) }\DIFaddend on
...
%DIF > \centerline{\includegraphics[width=4in,height=2.78in]{fig-partial-trust.pdf}}
\caption{
\DIFaddFL{Illustration of a trusted communication network and the network properties
used by the }{\em \DIFaddFL{bounded trust model}}\DIFaddFL{.
Edges represent mutually trusted communication links.
The sender (Alice, $s$) and receiver (Bob, $t$) trust all nodes
less than the }{\em \DIFaddFL{trust radius}} \DIFaddFL{$h$ hops away.
These nodes form their }{\em \DIFaddFL{trusted neighborhoods}} \DIFaddFL{$T_h(v)$ and $T_h(w)$.
We assume that all faults occur in the remaining nodes: the
}{\em \DIFaddFL{untrusted region}}\DIFaddFL{.
The untrusted nodes in contact with the trusted neighborhoods for the
}{\em \DIFaddFL{trust boundaries}} \DIFaddFL{$B_h(v)$ and $B_h(w)$,
which (in the absence of central bottlenecks) determine the
}{\em \DIFaddFL{effective redundancy}} \DIFaddFL{$\delta_h$ provided by the network.
...
\DIFaddend Now let $s \in V$ be an arbitrary sender
...
used by the }%DIFDELCMD < {\em %%%
\DIFdelFL{bounded trust model}%DIFDELCMD < }%%%
\DIFdelFL{.
Edges represent mutually trusted communication links.
The sender (Alice, $s$) and receiver (Bob, $t$) trust all nodes
less than the }%DIFDELCMD < {\em %%%
\DIFdelFL{trust radius}%DIFDELCMD < } %%%
\DIFdelFL{$h$ hops away.
These nodes form their }%DIFDELCMD < {\em %%%
\DIFdelFL{trusted neighborhoods}%DIFDELCMD < } %%%
\DIFdelFL{$T_h(v)$ and $T_h(w)$.
We assume that all faults occur in the remaining nodes: the
}%DIFDELCMD < {\em %%%
\DIFdelFL{untrusted region}%DIFDELCMD < }%%%
\DIFdelFL{.
The untrusted nodes in contact with the trusted neighborhoods for the
}%DIFDELCMD < {\em %%%
\DIFdelFL{trust boundaries}%DIFDELCMD < } %%%
\DIFdelFL{$B_h(v)$ and $B_h(w)$,
which (in the absence of central bottlenecks) determine the
}%DIFDELCMD < {\em %%%
\DIFdelFL{effective redundancy}%DIFDELCMD < } %%%
\DIFdelFL{$\delta_h$ provided by the network.
Alice and Bob can achieve the same level of attack-tolerance
as if they were directly connected by $\delta_h$ redundant channels.
}}
%DIFAUXCMD
%DIFDELCMD < %DIFDELCMD < \label{fig:trust-source}%%%
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{figure}[H]
\DIFaddendFL %\centerline{\includegraphics{fig-diameter}}
\caption{
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{figure}[H]
\DIFaddendFL %\centerline{\includegraphics[width=5in,height=5in]{fig-mincut}}
\caption{
Simulation of targeted attacks against a snapshot of the internet's router
network with a fraction of the edges rewired into a partial butterfly configuration.
...
\DIFaddbegin \begin{figure}[H]
\caption{
\DIFaddFL{Progression of place-within-level $z$ as the multipath routing algorithm
...
\begin{figure}[H]
\begin{center}
%DIF > \includegraphics{fig-routing}
\end{center}
\caption{
\DIFaddFL{An example of one path as constructed by the proposed multipath
routing algorithm.
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Progression of place-within-level $z$ as the multipath routing algorithm
cycles through the levels of the butterfly network.
}%DIFDELCMD < %DIFDELCMD < \label{fig:route-overview}%%%
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{An example of one path as constructed by the proposed multipath
routing algorithm.
The path is shown for $s = 10_2$
and $w = (6, 0110111_2)$.
}%DIFDELCMD < %DIFDELCMD < \label{fig:routing}%%%
...
\DIFaddbegin \subsection*{\DIFadd{Running Time}}
\DIFadd{At each node, the choice of edge is made using $O(1)$ lookups of the endpoint
...
\DIFdelbegin \DIFdel{Furthermore,
the existence }\DIFdelend \DIFaddbegin \DIFadd{Each node in a directed wrap-around butterfly network has an out-degree }\DIFaddend of \DIFaddbegin \DIFadd{2,
placing an upper bound of }\DIFaddend $2^h$ \DIFdelbegin \DIFdel{paths places a lower bound }\DIFdelend on the effective redundancy\DIFdelbegin \DIFdel{$\delta_h$,
showing that the }\DIFdelend \DIFaddbegin \DIFadd{,
which we have just shown the above algorithm achieves, so the bound is tight.
Thus, }\DIFaddend decentralized, redundant, structured networks such as the
...
\DIFdelbegin \DIFdel{This research was partly supported by the National Science Foundation under
Grant No. IIS-1617820}\DIFdelend \DIFaddbegin \DIFadd{The authors would also like to thank the reviewers for their helpful comments}\DIFaddend .

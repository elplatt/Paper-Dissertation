...
We are particularly interested in factors that give insight into the success of Wikipedia
as a large-scale collaboration, such as those relating to the network structure of
...
\begin{figure}[t!]
\begin{center}
\includegraphics[width=3in]{chapters/figures/WPProdPerf/fig-knitting.png}
\caption{
...
We now define a productivity measure which meets the above criteria.
...
For traditional organizations, maximum effective team size has been
variously estimated between 5 and 8 individuals
\cite{freeman_tyranny_1972, lohman_designing_2000, miflin_small_2004},
while WikiProject mean editor degrees range from 0 to 1256, with a median of 133.
...
The minimum $st$-cut between nodes $s$ and $t$ is the smallest set of edges that must be removed
for no path exists from $s$ to $t$.
The minimum cut (min-cut) of a graph is the smallest minimum $st$-cut over all node pairs $st$. 
The size (number of edges) of the graph min-cut quantifies the connectivity of a graph,
...
\begin{figure}[t!]
\centering
\includegraphics[width=6in,]{chapters/figures/WPProdPerf/fig-eff-perf-hist.pdf}
\caption{
...
\begin{figure}[t!]
\centering
\includegraphics[width=3in]{chapters/figures/WPProdPerf/fig-perf-prod}
\caption{
...
%\begin{figure}
%\centering
%\includegraphics[width=2.33in,height=2.33in]{chapters/figures/WPProdPerf/fig-degree-mincut.pdf}
%\caption{
...
For project similarity, we use the Jaccard similarity \cite{jaccard1912distribution},
which provides a meaningful index for comparing set membership.
\begin{description}
\setlength\itemsep{0pt}
\item[C-productivity]
(performance only).
...
\item[Mean similarity.] Mean Jaccard (by article) with other WikiProjects; a measure of topical complexity.
...
as it is highly correlated (r=0.44) with degree.
In many cases the min-cut is simply the removal of the highest-degree node.
...
with an average of 16.8 A-Class articles per project versus
118.2 and 251.1 for B-Class and C-Class.
The A-Class quality level is usually passed when an article reaches
good or featured article status,
which follow a different consensus process from other ratings.
...
Name          & Social stage & Individual stage & \makecell{Limited\\concern} & \makecell{Unknown\\objective} & \makecell{Single\\truth} \\
\hline
...
\begin{figure}[t!]
\centering
\includegraphics[width=6in,height=3in]{chapters/figures/WPProdPerf/fig-val-iter.pdf}
\caption{
...
Local strategies are more productive for the conform strategy and less productive for the best-neighbor strategy.
...
\centering
\begin{tabular}{lll}
Strategy & Perf. Std. Coeff. & Prod. Std. Coeff.\\
\hline
...
\begin{figure*}
\includegraphics[width=6.67in,height=1.33in]{chapters/figures/WPProdPerf/fig-deg-eff.pdf}
\includegraphics[width=6.67in,height=1.33in]{chapters/figures/WPProdPerf/fig-deg-perf.pdf}
\caption{
Performance and productivity of social learning strategies vs. mean network degree.
...
Our agent-based models offer several insights.

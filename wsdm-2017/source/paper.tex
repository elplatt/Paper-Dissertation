...
even if no single path between sender and receiver is fully trusted,
multiple message copies can be sent along different paths and compared
...
with the same effort required to compromise one of them).
The maximum number of such paths represents the effective redundancy that
can be utilized by any redundancy-based fault tolerance scheme.
We now propose a model for quantifying the effective redundancy of a network
as a function of its structure and the level of trust transitivity.
We will then use this model to evaluate a specific fault-tolerance scheme.

We now propose the {\em partial trust model}:
a technique for modeling communication between any two nodes
in an arbitrary network as communication across a number of
independent, direct links,
each of which might be compromised by an adversary.
Two links are considered {\em independent} if they do not
share a single point of failure.
Rather than assuming that trust is infinitely transitive (as in webs of trust),
this model assumes that trust is transitive up to a fixed number of hops.
The level of trust transitivity enters the model as a parameter called the
{\em trust radius}.
Any paths longer than this radius will contain some untrusted links,
hence {\em partial trust}.
The benefit of the partial trust model is its ability to abstract
away the details of the network structure,
while quantifying the amount of structural redundancy that can be
utilized by fault tolerance schemes.

To apply the partial trust model to a network,
we begin with an undirected graph $G = (V,E)$,
with nodes representing individual agents
and with edges representing mutual trust relationships
between those agents.
We define a {\em trust radius} $h$ and assume that trust is transitive
up to $h - 1$ hops away.
In other words, the nodes $v$ and $w$ trust each other if the distance between
$v$ and $w$, $d(v,w)$ is less than $h$.
For a given node $v$,
we call the set of nodes that $v$ trusts the
{\em trusted neighborhood} $T_h(v)$,
and all nodes at exactly distance $h$ from $v$ the
...
trusted by $v$,
but if compromised can entirely isolate $v$ from the rest of the network.

Using the above definitions, we now model communication between a
source $v$ (Alice) and a destination $w$ (Bob).
We assume that nodes trusted by {\em either} Alice or Bob will not
compromise the message.
In other words,
faults are assumed to occur only in the part of the network lying
between Alice and Bob's trust boundaries.
We call this part of the network the {\em untrusted region}.
